        ...
        try:
            ...
            if len(path_components) == 2 and path_components[0] == VIDEO_TO_STREAM_NAME and path_components[1] == "master.m3u8":
                ...
                try:
                    ...
                    # self.send_error(502, f"Bad Gateway: Could not fetch master M3U8")
                return
...
def main():
    ...
    global shutdown_requested, LOCAL_PROXY_PORT, WEBSOCKET_PORT
    ...
    # 新增客户端ID参数
    parser.add_argument('--client-id', type=int, default=1, help='Client ID (1, 2, or 3)')
    ...
    # 根据客户端ID设置不同的端口
    LOCAL_PROXY_PORT = 8082 + (client_id - 1) * 10  # 8082, 8092, 8102
    WEBSOCKET_PORT = 8083 + (client_id - 1) * 10    # 8083, 8093, 8103

    # 更新下载目录以区分不同客户端
    global DOWNLOAD_DIR
    DOWNLOAD_DIR = f"download_client_{client_id}"

    # 更新写入路径
    write_path = f"{write_path}_client_{client_id}"
    qoe_manager.set_write_path(write_path)
    
    logger.info(f"Starting Client {client_id} - Proxy Port: {LOCAL_PROXY_PORT}, WebSocket Port: {WEBSOCKET_PORT}")
